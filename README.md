...
- A TinyRFID reader on I2C bus `1` is polled for tag scans at address `0x13`; override with `OMB_TINYRFID_ADDRESS` if needed.
- RFID tags are stored in `data/rfid.sqlite3`, where each UID can be named and assigned to groups such as `red_crystal` for logic rules.
